...
1. **HTMLエンコード**: 翻訳対象テキストを `WebUtility.HtmlEncode` でエンコードし、既存の `<` や `>` 記号をエスケープします。
2. **タグのラッピング**: 正規表現 `(__.*?__)|(\[.*?\])|(\\n)` を用いて保護対象テキストを抽出し、`<keep> ... </keep>` で囲みます。
3. **API呼び出し**: DeepL API の `TextTranslateOptions` にて `TagHandling = "xml"` および `IgnoreTags = [ "keep" ]` を指定して翻訳リクエストを送信します。
4. **タグのアンラッピング**: APIからの翻訳結果から、すべての `<keep>` および `</keep>` タグを削除します。
5. **HTMLデコード**: `WebUtility.HtmlDecode` でエスケープされていた記号を元に戻し、最終的な翻訳結果とします。

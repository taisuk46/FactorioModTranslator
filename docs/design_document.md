...
Rust + Web技術ベースのクロスプラットフォーム対応アーキテクチャで構築。
...
| トリガー | ユーザーが「フォルダを選択」ボタンまたは「ZIPを選択」ボタンを押下する |

**基本フロー:**

| # | アクター | システム |
|---|---|---|
| 1 | 「フォルダを選択」ボタンまたは「ZIPを選択」ボタンを押下する | |
| 2 | | フォルダ選択時は `select_mod_path()`、ZIP選択時は `select_mod_zip_path()` を実行し、パス入力ダイアログを表示する |
| 3 | フォルダまたはZIPファイルのパスを選択する | |
...
| 事後条件 | ユーザーの編集内容がTranslationItemに反映され、Source=Manualに更新されていること |
...
| 6 | | TranslationItem の translated_text を更新し、source = "Manual" に設定する |

**代替フロー:**

| # | 条件 | 処理 |
|---|---|---|
| 4a | ユーザーがリバートボタンを押下 | 編集前の翻訳テキストに戻し、source を元の値に復元する |
...
| ユースケース名 | 翻訳ファイルを保存する |
| アクター | ユーザー |
| 概要 | プレビュー画面の翻訳結果を `.cfg` ファイルとして locale 構造でエクスポートする。ZIPソースの場合は保存形式を選択可能 |
...
|---|---|---|
| 1 | 保存ボタンを押下する | |
| 2 | | JS側から `invoke('save_translation', {...})` でRustバックエンドを呼び出す |
| 3 | | CfgParser が TranslationItem のリストを CfgFile に変換し `.cfg` フォーマットで書き出す |
| 4 | | `locale/{targetLang}/` ディレクトリ構造でファイルを出力する |
| 5 | | 保存完了通知をフロントエンドに返却・表示する |
...
| 1 | 保存ボタンを押下する | |
| 2 | | JS側から `invoke('save_translation', {...})` でRustバックエンドを呼び出す |
| 3 | | `source_type == Zip` を検出し、保存形式選択ダイアログを表示する |
| 4 | 保存形式を選択する（「フォルダに保存」または「ZIPに保存」） | |
| 5a | 「フォルダに保存」を選択: 保存先フォルダを選択する | |
| 5b | 「ZIPに保存」を選択: 保存先ファイルを選択する | |
| 6a | | フォルダ保存の場合: `locale/{targetLang}/` ディレクトリ構造で.cfgファイルを書き出す |
| 6b | | ZIP保存の場合: 元のZIPを読み取り、翻訳済み.cfgで差し替え、新ZIPファイルを作成する |
| 7 | | 保存完了通知をフロントエンドに返却・表示する |

**代替フロー:**

| # | 条件 | 処理 |
|---|---|---|
| 3a | 書込権限がない | Err文字列を返却し、フロントエンドがエラーメッセージを表示 |
| 3b | ディスク容量不足 | Err文字列を返却し、フロントエンドがエラーメッセージを表示 |
...
| 事前条件 | アプリケーションが起動済みであること |
| 事後条件 | 設定が `appsettings.json` に保存され、APIキーがOS Keyring (Windows Credential Manager) に保存されていること |
| トリガー | ユーザーが設定タブを選択する |
...
|---|---|---|
| 1 | 設定タブを選択する | |
| 2 | | JS側から `invoke('get_settings')` で現在の設定値を取得し画面に表示する |
| 3 | 翻訳エンジン（DeepL / Google Translate）を選択する | |
| 4 | 選択したエンジンのAPIキーを入力する | |
| 5 | Factorioのインストールパスを入力する | |
| 6 | 保存ボタンを押下する | |
| 7 | | JS側から `invoke('save_settings', {...})` および `invoke('save_api_key', { engine, key })` でRustバックエンドを呼び出す |
| 8 | | SettingsService がエンジン選択・パスを `appsettings.json` に保存する |
| 9 | | APIキーを `keyring` crate 経由でOS Keyringに保存する |
| 10 | | 保存完了通知を返却・表示する |
...
| # | 条件 | 処理 |
|---|---|---|
| 7a | テスト接続失敗 | Err文字列を返却し、フロントエンドがエラーメッセージを表示する |
| 5a | Factorioパスが無効 | 警告メッセージを表示する |
...
|---|---|---|
| 1 | 用語集タブを選択する | |
| 2 | | JS側から `invoke('get_glossary')` で現在の用語一覧を取得し表示する |
| 3 | 原語（source_term）と訳語（target_term）を入力する | |
| 4 | ソース言語・ターゲット言語を選択する | |
| 5 | 必要に応じて「翻訳対象から除外」チェックを設定する | |
| 6 | 追加ボタンを押下する | |
| 7 | | JS側から `invoke('add_glossary_entry', { entry })` でRustバックエンドを呼び出す |
| 8 | | GlossaryService が新しいエントリを `glossary.json` に追加・保存する |
...
| 1 | 一覧から削除対象の用語を選択する | |
| 2 | 削除ボタンを押下する | |
| 3 | | JS側から `invoke('remove_glossary_entry', { term })` でRustバックエンドを呼び出す |
| 4 | | GlossaryService が用語を削除し `glossary.json` に保存する |

**代替フロー:**

| # | 条件 | 処理 |
|---|---|---|
| 6a | 同一の原語が既に登録済み | 確認ダイアログを表示し、上書きするかユーザーに確認する |
...
| 事前条件 | アプリケーションが起動済みであること。過去に翻訳が実行されていること |
| 事後条件 | なし（参照のみ） |
| トリガー | ユーザーが履歴タブを選択する |
...
|---|---|---|
| 1 | 履歴タブを選択する | |
| 2 | | JS側から `invoke('get_history')` でSQLiteから履歴一覧を取得し表示する |
| 3 | Mod名、キー等で検索・フィルタを行う | |
| 4 | | フィルタ条件に一致する履歴をJS側で絞り込み表示する |
...
| 2b | SQLiteデータベースが破損 | Err文字列を返却し、データベース再作成を提案する |
...
| 事前条件 | 設定画面でFactorioのインストールパスが設定済みであること |
| 事後条件 | マッチしたエントリにバニラ訳語が適用され、source が "VanillaKeyMatch" または "VanillaTextMatch" に設定されていること |
...
| 6 | いずれもマッチしなかった場合、バニラ訳語の参考情報を取得し、翻訳APIへの入力に付加する |
...
| 事前条件 | アプリケーションが起動済みであること |
| 事後条件 | UIの全文字列が選択した言語で表示され、設定が保存されていること |
| トリガー | ユーザーが言語切替ボタンを操作する |

**基本フロー:**

| # | アクター | システム |
|---|---|---|
| 1 | 言語切替ボタンで切替先の言語を選択する | |
| 2 | | JS側から `invoke('get_localized_strings', { lang })` でRustバックエンドを呼び出す |
| 3 | | LocalizationService が `locales.json` から選択言語の文字列リソースを返却する |
| 4 | | JS側で全UI要素のテキストを動的に差し替える |
| 5 | | 選択した言語を `appsettings.json` に保存する |

**代替フロー:**

| # | 条件 | 処理 |
|---|---|---|
| 3a | 言語リソースが見つからない | デフォルト言語（日本語）にフォールバックし、警告を表示する |
...
classDiagram
    ...
    class Commands {
        ...
        +select_mod_zip_path() Result~Option~String~~
...
| Mod選択タブ | Modの読込と翻訳実行。フォルダ選択ボタンとZIP選択ボタンの2つを提供 | パス入力、翻訳実行 |
...
| ZIP | ZIPファイル破損、展開不可 | Err文字列を返却し、フロントエンドがエラーメッセージを表示 |

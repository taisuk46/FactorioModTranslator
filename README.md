...
### ビルド（デバッグ用実行ファイルの生成）
通常のビルドでは、デバッグ情報を含んだ実行ファイルが生成されます。

```bash
dotnet build
```
生成物：`src/FactorioModTranslator/bin/Debug/net8.0-windows/FactorioModTranslator.exe`

### デバッグ用パッケージの作成
リリース用と同様に、デバッグ情報を保持したまま特定のフォルダへシングルファイルとして出力したい場合は以下のコマンドを使用します。

```bash
dotnet publish src/FactorioModTranslator/FactorioModTranslator.csproj -c Debug -r win-x64 --self-contained true -p:PublishSingleFile=true -p:PublishReadyToRun=false -p:IncludeNativeLibrariesForSelfExtract=true -o ./publish_debug
```
...
```bash
dotnet publish src/FactorioModTranslator/FactorioModTranslator.csproj -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true -p:PublishReadyToRun=false -p:IncludeNativeLibrariesForSelfExtract=true -o ./publish
```

### 生成物の場所
上記コマンド実行後、`publish/` フォルダ内に **`FactorioModTranslator.exe`** が作成されます。
このファイルは「自己完結型」としてビルドされているため、.NET ランタイムがインストールされていない Windows 環境でも単体で動作します。
